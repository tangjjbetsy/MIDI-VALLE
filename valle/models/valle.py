...
from .bert import ExpressionBert, ExpressionBertLM, ExpressionCrossEntropyLoss
...
class VALLF(nn.Module):
    ...
    def __init__(
        ...
        self.vocab_sizes = [92, 68, 1156, 772, 388, 20] # set the bar to 30 due to the shift
        emb_sizes = [32, 32, 256, 128, 64, 32]
        
        # self.vocab_sizes = [92, 68, 1156, 772]
        # emb_sizes = [32, 32, 256, 128]
        
        self.feat_emb = []
        for i in range(len(self.vocab_sizes)):
            self.feat_emb.append(TokenEmbedding(emb_sizes[i], self.vocab_sizes[i]))
    ...
    def _prepare_prompts(self, y, y_lens, codes, nar_stage, y_prompts_codes):
        ...
        if self.prefix_mode == 0:
            ...
            #NOTE(Jingjing): Replace 225 by 150 due to the frame rate change [MusicGen Enocder] has 
            # the frame rate at 50Hz with audios sampled at 32kHz, stride = 640
            prefix_len = min(prefix_len, 150)  # 24000/320 * 3s = 225 frames
            ...
            if self.prefix_mode == 2:
                # random prefix
                #NOTE(Jingjing): Replace 225 by 150 due to the frame rate change
    ...
    def forward(
        ...
        **kwargs,
        ...
        x = self.ar_text_embedding(embs) # --> Expression tokeniser
        ...
        if train_stage in [0, 2]:
            ...
            x = self.nar_text_embedding(embs) # --> Expression tokeniser
...
class VALLE(VALLF):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # AR Decoder
        if train_stage in [0, 1]:
            # x = self.ar_text_embedding(hidden_states)
            x = self.ar_text_embedding(embs)
            # x = self.ar_text_embedding(text)
            x = self.ar_text_prenet(x)
            x = self.ar_text_position(x)
        ...
        if train_stage in [0, 2]:
            ...
            # x = self.nar_text_embedding(hidden_states)
            x = self.nar_text_embedding(embs)
            # x = self.nar_text_embedding(text)
            x = self.nar_text_prenet(x)
            x = self.nar_text_position(x)
    ...
    def continual(
        ...
        prefix_len = min(int(y.shape[1] * 0.5), 3 * 50)
        # prefix_len = y.shape[1]
...
def clip_feature(tokens):
    vocab_sizes = [92, 68, 1156, 772, 388, 20]
    for i in range(tokens.shape[-1]):
        tokens[..., i] = torch.clamp(tokens[..., i], min=0, max=vocab_sizes[i]-1)
    return tokens

...
class ExpressionBert(nn.Module):
    def __init__(
        ...
        output_features: list = ["Velocity", "IOI", "Duration"] ,
        ...
        vocab_names: list = ["Pitch", "Velocity", "Duration", "IOI", "Position", "Bar"],
        ...
        vocab_size: dict = {"Pitch": 92,
                            "Velocity": 68,
                            "Duration": 1156,
                            "IOI": 772,
                            "Position": 388,
                            "Bar": 3004},
        emb_size: dict = {"Pitch": 32,
                        "Velocity": 64,
                        "Duration": 256,
                        "IOI": 256,
                        "Position": 128,
        ...
        self.style_is_condition = condition
...
class ExpressionBertLM(nn.Module):
    ...
    def forward(self, x, mask, style, condition=True):
        # feed to bert
        y = self.bert(x, mask)
        y = y.hidden_states[-1]

        if self.bert.style_is_condition and condition: 
            # style = torch.zeros_like(style, dtype=torch.long)  # for fine-tuning only
...
class ExpressionCrossEntropyLoss(nn.Module):
    def __init__(self, output_features, vocab_size, label_smoothing=None):
        super().__init__()
        if label_smoothing != None:
            self.cross_entropy = nn.CrossEntropyLoss(reduction="none", label_smoothing=label_smoothing)
        else:
    ...
    def forward(self, x, y, mask):
        ...
        total_loss = sum([losses[feat]["loss"] for feat in losses.keys()]) / len(self.out_feat)

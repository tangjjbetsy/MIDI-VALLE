...
def compute_loss(
    ...
    if params.model_name.lower() in ["bert-valle", "bertvalle"]:
        # Convert list of numeric strings to a tensor of long integers
        style = torch.tensor([int(lang) for lang in batch['language']], dtype=torch.long).to(device)
        seq_len = midi_tokens.size(1)
        mask = (torch.arange(seq_len, device=midi_tokens.device)
            .unsqueeze(0)
            .expand(midi_tokens.size(0), seq_len) < midi_tokens_lens.unsqueeze(1)).int()
    
    # assert midi_tokens.ndim == 2
...
def run(rank, world_size, args):
    ...
    if params.model_name.lower() in ["bert-valle", "bertvalle"]:
        checkpoints = load_checkpoint_if_available(
            params=params, model=model.valle, model_avg=model_avg
        )
        print(model.valle)
        # Load the checkpoint file first
        bertlm_ckpt_path = "/data/home/acw555/score-to-performance/logs/s2p_bert_class/runs/2024-05-22_02-26-21/checkpoints/model_state_dict.pth"
        # Load state dict and remove "net." prefix from keys
        bertlm_state_dict = torch.load(bertlm_ckpt_path)
        bertlm_state_dict = {k.replace('net.', ''): v for k, v in bertlm_state_dict.items()}

        # Now load the state dict into the model
        # Load BERT LM parameters and verify loading
        model.bertlm.load_state_dict(bertlm_state_dict, strict=False)
        # Check if parameters are loaded correctly
        # for name, param in model.bertlm.named_parameters():
        #     if name in bertlm_state_dict:
        #         loaded_param = bertlm_state_dict[name]
        #         if torch.equal(param.data, loaded_param):
        #             logging.info(f"Parameter {name} loaded correctly")
        #         else:
        #             logging.warning(f"Parameter {name} not loaded correctly")
        #     else:
        #         logging.warning(f"Parameter {name} not found in checkpoint")
    ...
    if params.model_name.lower() in ["bert-valle", "bertvalle"]:
        if params.train_stage:
            ...
            model_parameters = model.valle.parameters()
    else:     
        if params.train_stage:
        ...
        if params.model_name.lower() in ["bert-valle", "bertvalle"]:
            if params.train_stage:  # != 0
                ...
                parameters_names.append(
                    [
                        name_param_pair[0]
                        for name_param_pair in model.valle.named_parameters()
                    ]
                )
        else:
            if params.train_stage:  # != 0
    ...
    test_cuts = filter_short_and_long_utterances(
        test_cuts, params.filter_min_duration, params.filter_max_duration
    )

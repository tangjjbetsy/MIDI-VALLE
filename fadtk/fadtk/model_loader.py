...
class CustomModel(ModelLoader):
    def __init__(self):
        super().__init__('encodec-music', 128, 32000)
        self.device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        
    def load_model(self):
        from audiocraft.models import CompressionModel
        self.model = CompressionModel.get_pretrained('egs/atepp/checkpoints/compression_32khz_new.bin')
        self.model.to(self.device)

    def _get_embedding(self, audio: np.ndarray) -> np.ndarray:
        encoded_frames, scales = self.model.encode(audio.to(self.device))
        encoded_frames = self.model.decode_latent(encoded_frames)
        # Concatenate
        encoded_frames = encoded_frames.squeeze().T # [timeframes, 128]
        return encoded_frames
